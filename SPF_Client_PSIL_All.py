# -*- coding: utf-8 -*-
"""
Created on Fri Sep 22 11:20:43 2023
...
layout = [  [sg.Text('Welcome to SPF Maker')],
            ...
            [sg.Text('Min AADT'), sg.InputText()],
            [sg.Text('Min Segment Length'), sg.InputText()],
            [sg.Text('Output Folder'), sg.InputText()],
...
while x<1:
    ...
    AccType = values[0]
    MaxSample = int(values[1])
    MinAADT = max(float(values[2]),1)
    MinL = float(values[3])
    FolderName = values[4]
    ...
    Groups = [
              #['1','404','404','404','404','404'], ['2','404','404','404','404','404'], 
              #['3','4','5','6','7','8'], X
              #['9','404','404','404','404','404'], 
              #['10','12','404','404','404','404'], X
              #['11','404','404','404','404','404'],
              #['13','404','404','404','404','404'], ['14','16','18','404','404','404'], X
              #['15','17','404','404','404','404'], 
              #['19','404','404','404','404','404'], X
              #['20','404','404','404','404','404'], ['21','32','34','404','404','404'], X
              #['22','404','404','404','404','404'], X
              #['23','404','404','404','404','404'], X
              #['24','404','404','404','404','404'], X
              #['25','404','404','404','404','404'], X
              #['26','404','404','404','404','404'], ['27','404','404','404','404','404'], X
              #['28','404','404','404','404','404'], ['29','404','404','404','404','404'],
              #['30','404','404','404','404','404'], ['31','404','404','404','404','404'],
              #['33','404','404','404','404','404'], 
              #['35','404','404','404','404','404'], X
              ['36','404','404','404','404','404'], 
              #['37','404','404','404','404','404'], ['38','40','404','404','404','404'], X
              #['39','404','404','404','404','404'], X
              ['41','404','404','404','404','404'], ['42','404','404','404','404','404'],
              ['43','404','404','404','404','404']]
    ...
    for HwyClassGroup in Groups:
        ...
        DB_filtered = DB_output[(DB_output['HwyClass'] == int(HwyClassGroup[0])) |
                                (DB_output['HwyClass'] == int(HwyClassGroup[1])) |
                                (DB_output['HwyClass'] == int(HwyClassGroup[2])) |
                                (DB_output['HwyClass'] == int(HwyClassGroup[3])) |
                                (DB_output['HwyClass'] == int(HwyClassGroup[4])) |
                                (DB_output['HwyClass'] == int(HwyClassGroup[5]))]
        DB_flitered = DB_filtered[DB_filtered['AvgAADT'] >= MinAADT]
        DB_filtered = DB_filtered[DB_filtered['SegmentLength'] > MinL]
        DB_filtered = DB_filtered[DB_filtered['AvgAADT'] >= 1].sort_values(by='AvgAADT')
        DB_filtered = DB_filtered.reset_index()
        #Eliminates extreme outliers of very short segment length relative to accident counts.
        if AccType == 'TotalInjury':
            Ratios = np.divide(DB_filtered['TotalCrashes']-DB_filtered['PDO'],DB_filtered['SegmentLength'])
            DB_filtered = DB_filtered[np.divide(DB_filtered['TotalCrashes']-DB_filtered['PDO'],DB_filtered['SegmentLength']) < np.mean(Ratios) + 3*np.std(Ratios)]
        else:
            Ratios = np.divide(DB_filtered[AccType],DB_filtered['SegmentLength'])
            DB_filtered = DB_filtered[np.divide(DB_filtered[AccType],DB_filtered['SegmentLength']) < np.mean(Ratios) + 3*np.std(Ratios)]
        #If user set a max sample size smaller than the full size of the sample, selects a 
        #sample of that size mostly at random but always containing the 10 largest AvgAADT values
        #or all values more than 3 standard deviations above the mean, whichever is more. 
        print('filtered database')
        if len(DB_filtered['AvgAADT']) > MaxSample:
            OutThr = np.mean(DB_filtered['AvgAADT'])+3*np.std(DB_filtered['AvgAADT'])
            print('calculated OutThr')
            OutLen = max(len(DB_filtered[DB_filtered['AvgAADT'] > OutThr]['AvgAADT']),10)
            print('calculated OutLen')
            RandSamp = random.sample(range(len(DB_filtered['AvgAADT'])-OutLen), MaxSample-OutLen)
            RandSamp.sort()
            L_list = []
            AADT_list = []
            Acc_list = []
            ...
            for n in range(OutLen):
                L_list.append(DB_filtered['SegmentLength'][len(DB_filtered['AvgAADT'])-OutLen+n])
                AADT_list.append(DB_filtered['AvgAADT'][len(DB_filtered['AvgAADT'])-OutLen+n])
                Acc_list.append(DB_filtered[AccType][len(DB_filtered['AvgAADT'])-OutLen+n])
            Data = pd.DataFrame({'L' : L_list, 'AADT' : AADT_list, AccType : Acc_list})
            print('produced data frame')
        else:
            if AccType == 'TotalInjury':
        ...
        OutputCSV = './'+FolderName+'/'
        for Code in HwyClassGroup:
            if Code != '404':
                OutputCSV += Code+'-'
        OutputCSV += AccType+'.xlsx'
